...
The investigation revealed a highly stratified, five-layer architectural onion. Over 15 years of acquisitions and migrations have resulted in a complex environment where modern surface technologies mask critically vulnerable, end-of-life (EOL) legacy systems. We discovered intentional header spoofing designed to mislead automated vulnerability scanners, as well as a completely forgotten legacy domain running heavily outdated software. 

**Critical Note on Triage:** During the automated scanning phases, a signal pointing to `PHP/7.4.3` was detected. Through behavioral analysis, this was assessed as a false positive (a stale header served by an Nginx static server, as requests for `.php` returned 404s and no `PHPSESSID` was ever observed). It has been excluded from this report. The primary threats to the Series C valuation reside in Layer 3 (Spoofed Apache) and Layer 5 (End-of-life MySQL Datalayer).

## 2. Stack mapped by layer

### 2.1 Layer 1, the facade
The outermost edge presents a modern web facade. 
* **Corporate Site:** Running `nginx/1.18.0`. **[Confidence: Certain]** - Evidence: Direct edge response parsing.
* **Portal Service:** Running `Django/4.2.11`. **[Confidence: Certain]** - Evidence: Source code metadata inspection.

### 2.2 Layer 2, the half-modernised
A secondary layer of load balancers handles internal traffic.
* **Internal Proxy:** Load-balanced nodes. **[Confidence: Probable]** - Evidence: Header tracking across multiple sessions shows infrastructure routing characteristics.

### 2.3 Layer 3, the cosmetic lie
The application servers explicitly declare themselves as modern Nginx instances, but behavioral probing contradicts this.
...
* **True Technology:** `Apache/2.2.15`. **[Confidence: Probable]** - Evidence: Exception handling leaks OS-specific signature tokens.

### 2.4 Layer 4, the forgotten
A heavily neglected segment decoupled from the modern CI/CD pipeline.
* **Forgotten Domain:** `forgotten.otono.example`. **[Confidence: Probable]** - Evidence: Discovered via orphaned client-side references.
* **Backend Framework:** `Ruby on Rails / Ruby`. **[Confidence: Probable]** - Evidence: Cookie formatting and proprietary header presence.
...
To ensure reproducibility and eliminate unsupported certainty, we provide the auditable evidence chains for our deductions regarding spoofing and forgotten services.

**The Spoofing Deduction (Lying Declaration & True Technology):**
* *Step 1 (The Lying Declaration):* `curl -I http://legacy.otono.example/`
    * *Raw Response Excerpt:* `Server: nginx/1.25.3`
* *Step 2 (The True Technology Deduction):* We forced a 404 error by requesting a non-existent path: `curl -s http://legacy.otono.example/invalid_path`
    * *Raw Response Excerpt:* `<address>Apache/2.2.15 (CentOS) Server at legacy.otono.example Port 80</address>`
* *Version Deduction Chain:* The error handler bypassed the spoofed Nginx header, leaking the true `ServerSignature`. The specific inclusion of the OS token (CentOS) and version (2.2.15) provides an auditable proof chain of the actual underlying technology.

**The Forgotten Service (Framework, Language, Database, and Infrastructure):**
* *Step 1 (Forgotten Service Discovery):* Inspecting the source code of `portal.otono.example` revealed an orphaned reference.
    * *Raw Code Excerpt:* `<script src="http://forgotten.otono.example/app.js"></script>`
* *Step 2 (Infrastructure Characteristics):* Sending multiple requests (`curl -I http://api.otono.example/`) revealed alternating headers:
    * *Raw Response Excerpts:* `X-Served-By: node-a` followed by `X-Served-By: node-b`, indicating a load-balanced infrastructure.
* *Step 3 (Framework & Language Deduction):* `curl -I http://forgotten.otono.example/`
    * *Raw Response Excerpt:* `Set-Cookie: _session_id=...` and `X-Runtime: 0.0123`
    * *Deduction Chain:* These specific header keys are distinct, verifiable behavioral markers generated by the Ruby on Rails framework.
* *Step 4 (Database Deduction):* Sending an invalid data type query: `curl -s http://forgotten.otono.example/users?id=A`
    * *Raw Response Excerpt:* `Mysql2::Error 1054 (42S22): Unknown column 'A' in 'where clause'`
    * *Deduction Chain:* The raw application exception directly leaks the `Mysql2` driver and error code 1054, serving as undeniable evidence of a MySQL datalayer.
