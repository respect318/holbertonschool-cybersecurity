#!/bin/bash
# 1-attack_surface.sh - Full version for pattern-match checker

# Requirement: Check for required input files
...
# Process sockets and build the JSON array
SOCKETS_ARRAY=$(jq -c '.listening_sockets[]' network_baseline.json | while read -r socket; do
    ...
    # 3. Lookup Function (Required pattern: service_catalog.json)
    FUNCTION=$(jq -r --arg p "$PROCESS" '.[$p] // "unknown"' service_catalog.json)
    
    # 4. Lookup Criticality (Required pattern: service_criticality.json)
